...
def makepayment(request):
    ...
    o=Order.objects.filter(id=request.user.id)
    s=0
    for i in o:
        s=s+i.amt
...
def paymentsuccess(request):
    ...
    #     i.delete()
    sub='Order Status'
    msg='<h1>Order Placed Succesfully...!!</h1>'
    frm='prajyotthorat161@gmail.com'
    u=User.objects.filter(id=request.user.id)
    to=u[0].email
...
#     o=OrderHistory.objects.filter(id=request.user.id)
#     context={}
#     context['data']=o
